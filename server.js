...
// Mapping of stored video links and pop-up library files
...
wss.on('connection', ws => {
  ...
  ws.on('message', message => {
    ...
    if (parsedMessage.type === 'new_video') {
      ...
      if (!videoLibraryMapping[currentVideoLink]) {
        currentLibraryFile = getNextFilename();
        videoLibraryMapping[currentVideoLink] = currentLibraryFile;
      } else {
        currentLibraryFile = videoLibraryMapping[currentVideoLink];
      }
      ...
      if (currentLibraryFile) {
        ...
        wss.clients.forEach(client => {
          if (client.readyState === WebSocket.OPEN) {
            client.send(JSON.stringify({ type: 'new_danmaku', data: danmakuMessage }));
          }
        });
      }
    }
